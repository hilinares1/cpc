...
class ImportProducts(models.TransientModel):
    ...
    def _read_csv_data(self, binary_data):
        ...
        """
        data = csv.reader(
            StringIO(base64.b64decode(self.file_path).decode("utf-8")),
            quotechar='"',
            delimiter=",",
        )
    ...
    def do_import_product_data(self):
        ...
        if not file_path or file_path == "":
            _logger.warning(
                "Import can not be started. Configure your schedule Actions."
            )
            return True
        ...
        try:
            ...
            _logger.warning(
                "Can not read source file(csv) '%s', Invalid file path or File not reachable on file system."
                % (file_path)
            )
            return True
        if not data_lines:
            _logger.info(
                "File '%s' has no data or it has been already imported, please update the file."
                % (file_path)
            )
            return True
        ...
        for data in data_lines:
            ...
            try:
                ...
                if data["Product Category"]:
                    internal_categ_id = product_category_obj.search(
                        [("name", "=", categ_id)]
                    )
                    if not internal_categ_id:
                        internal_categ_id = product_category_obj.create(
                            {"name": categ_id}
                        )
                else:
                    internal_categ_id = product_category_obj.search(
                ...
                if color:
                    for cl in color.split("/"):
                        attribute_color_value_id = attribute_value_obj.search(
                            [
                                ("name", "=", cl),
                                ("attribute_id", "=", attribute_color_id.id),
                            ]
                        )
                        if attribute_color_value_id:
                            attribute_color_value_id_lis.append(
                                attribute_color_value_id.id
                            )
                        else:
                            attribute_color_value_id = attribute_value_obj.create(
                                {"attribute_id": attribute_color_id.id, "name": cl}
                            )
                            attribute_color_value_id_lis.append(
                                attribute_color_value_id.id
                            )
                ...
                if size:
                    for sz in size.split("/"):
                        attribute_size_value_id = attribute_value_obj.search(
                            [
                                ("name", "=", sz),
                                ("attribute_id", "=", attribute_size_id.id),
                            ]
                        )
                        if attribute_size_value_id:
                            attribute_size_value_id_lis.append(
                                attribute_size_value_id.id
                            )
                        else:
                            attribute_size_value_id = attribute_value_obj.create(
                                {"attribute_id": attribute_size_id.id, "name": sz}
                            )
                            attribute_size_value_id_lis.append(
                                attribute_size_value_id.id
                            )
                ...
                if attribute_color_value_id:
                    attribute_value_list.append(
                        [
                            ...
                            {
                                "attribute_id": attribute_color_id.id,
                                "value_ids": [(6, 0, attribute_color_value_id_lis)],
                            },
                        ]
                    )
                if attribute_size_value_id:
                    attribute_value_list.append(
                        [
                            ...
                            {
                                "attribute_id": attribute_size_id.id,
                                "value_ids": [(6, 0, attribute_size_value_id_lis)],
                            },
                        ]
                    )

                exist_product_template = product_tmpl_obj.search(
                    [("default_code", "=", default_code)]
                )

                product_barcode = False
                if barcode:
                    product_barcode = self.env["product.product"].search(
                        [("barcode", "=", barcode)]
                    )
                ...
                if len(exist_product_template.ids) != 1:
                    for dup in exist_product_template:
                        duplicate_product_template.append(
                            {"default_code": dup.default_code}
                        )
                    exist_product_template_test = product_tmpl_obj.search(
                        [("barcode", "=", barcode)]
                    )
                ...
                if exist_product_template:
                    if attribute_color_id:
                        exist_color_line = (
                            exist_product_template.attribute_line_ids.filtered(
                                lambda a: a.attribute_id.id == attribute_color_id.id
                            )
                        )
                    ...
                    if attribute_size_id:
                        exist_size_line = (
                            exist_product_template.attribute_line_ids.filtered(
                                lambda a: a.attribute_id.id == attribute_size_id.id
                            )
                        )
                        exist_size_line.value_ids = attribute_size_value_id_lis
                        exist_product_template.create_variant_ids()
                    for variant in exist_product_template.product_variant_ids.filtered(
                        lambda self: self.id not in variant_list
                    ):
                        stock_move_line_list.append(
                            [
                                0,
                                0,
                                {
        ...
        if stock_move_line_list:
            stock_inventory = stock_inventory_obj.create(
                {"name": "Bulk Import", "line_ids": stock_move_line_list}
            )
